...
#1.1 UPDATE: CUSTOM LEVEL PACKS
    #Now you can make your own custum level packs and not have to fuss with
    #the level menu code crap! Each pack can contain 99 levels, and the
    #base game can currently support unlimited packs.
    #(I'm bad at math, but by my reckoning, if you actually do that, your Thumby will be full.)
    #Also, completely overhauled the level page system, it's much better now.

#FAQ
    #1. How do I make custom levels?
        #A link to the tutorial can be found packaged with the game files,  "LevelCreationGuide.txt" 
    ...
    #5. Can I play this on the code.thumby.us Emulator?
        #As of right now, no. The emulator can't write to save data, which is requried.

#TO DO:
    #Put it on GitHub and Itch.io
    #Put in a new Pull Request to get it on the Thumby Arcade
...
if (thumby.saveData.hasItem('temp')) == False:
    ...
    thumby.saveData.setItem('levelFile', 'standard')
    thumby.saveData.setItem('custom', False)
    thumby.saveData.setItem('packName', "none")
    thumby.saveData.save()
if (thumby.saveData.hasItem('temp')) == True:
    temp = (thumby.saveData.getItem('temp'))
    levelFile = (thumby.saveData.getItem('levelFile'))
    print(temp)
    print(levelFile)
...
pointerSpr = thumby.Sprite(5, 5, pointerMap, key = 0)#Level selection pointer
pointerSprMirror = thumby.Sprite(5, 5, pointerMap, key = -1, mirrorX = 1)#Literally only shows up on title screen
pointerSpr2 = thumby.Sprite(5, 5, pointerMap, key = 0)#Main Menu pointer
pointerSpr3 = thumby.Sprite(5, 5, pointerMap, key = 0)#Sound On/Off pointer
pointerSpr4 = thumby.Sprite(5, 5, pointerMap, key = 0)#Custom or Default pointer
pointerSpr5 = thumby.Sprite(5, 5, pointerMap, key = 0)#Custom level pack pointer
...
def loadLevel():
    ...
    temp = thumby.saveData.getItem('temp')
    levelFile = thumby.saveData.getItem('levelFile')
    isCustom = thumby.saveData.getItem('custom')
    packName = thumby.saveData.getItem('packName')
    y = os.listdir
    if isCustom == False:
        levelString = "/Games/PogoPicross/Levels/"+str(levelFile)+"/level"+str(temp)+".json"
    if isCustom == True:
        levelString = "/Games/PogoPicross/Levels/"+str(levelFile)+"/"+str(packName)+"/level"+str(temp)+".json"
...
#logic for the pointer in customLevelPack()
def customPointerMovement():
    moveNumY = 6
    moveNumX = 0
    if thumby.buttonU.justPressed():
        buttonMoveSound()
        pointerSpr5.y -= moveNumY
    if thumby.buttonD.justPressed():
        buttonMoveSound()
        pointerSpr5.y += moveNumY
    if thumby.buttonR.justPressed():
...
def printLevelNumbers():
    global pageOffset
    pageOffset = (levelPage * 24) - 23
    printNumber = pageOffset
    printSpaceX = 5
    printSpaceY = 9
    pageLimit = pageOffset + 23
    if levelPage == 1:
        pageOffset = 1 #this should match the top left level number on the screen for later math to work
    if printNumber <= (levelPage * 24) and printNumber <= packLevels:
        fontSmall()
        while printNumber <= packLevels:
            thumby.display.drawText(str(printNumber), int(printSpaceX), int(printSpaceY), 1)
            printNumber += 1
            printSpaceX += 12
            if printNumber == 7 + pageOffset - 1:
                printSpaceY = 15
                printSpaceX = 5
            if printNumber == 13 + pageOffset - 1:
                printSpaceY = 21
                printSpaceX = 5
            if printNumber == 19 + pageOffset - 1:
                printSpaceY = 27
                printSpaceX = 5
...
def levelMenu(): #Level selection menu
    ...
    packName = thumby.saveData.getItem('packName')
    if isCustom == False:
        filePath = "/Games/PogoPicross/Levels/standard/packSize.json"
    if isCustom == True:
        filePath = "/Games/PogoPicross/Levels/custom/" + str(packName) +"/packSize.json"
    x = open(str(filePath))
    data = json.load(x)
    global packLevels
    packLevels = int(data["packLevels"])
    global pageTotal
    pageTotal = (packLevels // 24) + 1
    print("Page total =", pageTotal)
    x.close()
    waiting3 = True
    pointerSpr.x = 1
    pointerLevel = 0
    ...
    while(waiting3):
        ...
        if isCustom == True:
            fontMed()
            thumby.display.drawText(packName.upper(), 0, 0, 1)
        fontSmall()
        thumby.display.drawText("Next", 5, 33, 1)
        thumby.display.drawText("Prev", 30, 33, 1)
        thumby.display.drawText("B:Back", 48, 33, 1)
        printLevelNumbers()
        ...
        if thumby.buttonA.justPressed():
            ...
            if pointerSpr.x == 1 and pointerSpr.y ==33:
                if levelPage == pageTotal:#<----------- THIS VARIABE RIGHT HERE. Increase to add more pages.
                    errorNoise()
                else:
...
def customLevelPack():
    ...
    global packPage
    packPage = 0
    global packPageOffset
    packPageOffset = 0
    while(waiting4):
        x = os.listdir("/Games/PogoPicross/Levels/custom/")
        customPointerMovement()
        thumby.display.fill(0)#draw text
        thumby.display.setFPS(60)
        decoSpr3.y = 1
        decoSpr3.x = 8
        ...
        thumby.display.drawSprite(decoSpr4)
        fontMed()
        thumby.display.drawText("CUSTOM", 19, 0, 1)
        thumby.display.drawSprite(pointerSpr5)
        fontSmall()
        thumby.display.drawText("Next", 5, 33, 1)
        thumby.display.drawText("Prev", 30, 33, 1)
        thumby.display.drawText("B:Back", 48, 33, 1)
        try:
            fontSmall()
            thumby.display.drawText(x[0 + packPageOffset], 6, 9, 1)
            thumby.display.drawText(x[1 + packPageOffset], 6, 15, 1)
            thumby.display.drawText(x[2 + packPageOffset], 6, 21, 1)
            thumby.display.drawText(x[3 + packPageOffset], 6, 27, 1)
            testNum += 1
            thumby.display.update()
        except:
            thumby.display.update()
        ...
        if thumby.buttonA.justPressed():
            if pointerSpr5.y == 9:
                try:
                    thumby.saveData.setItem('packName', x[0+packPageOffset])
                    thumby.saveData.save()
                    buttonAsound()
                    levelMenu()
                except:
            ...
            if pointerSpr5.y == 15:
                try:
                    thumby.saveData.setItem('packName', x[1+packPageOffset])
                    thumby.saveData.save()
                    buttonAsound()
                    levelMenu()
                except:
            ...
            if pointerSpr5.y == 21:
                try:
                    thumby.saveData.setItem('packName', x[2 + packPageOffset])
                    thumby.saveData.save()
                    buttonAsound()
                    levelMenu()
                except:
            ...
            if pointerSpr5.y == 27:
                try:
                    thumby.saveData.setItem('packName', x[3 + packPageOffset])
                    thumby.saveData.save()
                    buttonAsound()
                    levelMenu()
                except:
                    errorNoise()
            if pointerSpr5.y == 33 and pointerSpr5.x == 0:#Next
                    packPage += 1
                    packPageOffset = packPage * 4
                    thumby.display.update()
                    print(packPage)
                    print(packPageOffset)
            if pointerSpr5.y == 33 and pointerSpr5.x == 24:#Prev
                if packPage == 0:
                    errorNoise()
                else:
                    packPage -= 1
...
def customOrNot():
    ...
    while(waiting2):
        ...
        thumby.display.drawText("LEVELS", 19, 0, 1)
        fontSmall()
        thumby.display.drawText("Standard", 20, 9, 1)
        thumby.display.drawText("Custom", 20, 15, 1)
        thumby.display.drawText("B:Back", 48, 33, 1)
        thumby.display.drawSprite(pointerSpr4)
        thumby.display.update()
        if pointerSpr4.y > 15:
            pointerSpr4.y = 9
        if pointerSpr4.y < 9:
        ...
        if thumby.buttonA.justPressed():
            if pointerSpr4.y == 9:
                ...
                levelMenu()
            if pointerSpr4.y == 15:
                levelFilePath = "custom"
                thumby.saveData.setItem("levelFile", "custom")
                thumby.saveData.setItem("custom", True)
                thumby.saveData.save()
                customLevelPack()
