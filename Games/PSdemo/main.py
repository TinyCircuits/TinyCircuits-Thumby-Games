...
async def main():
	...
	import thumby
	await thumby.display.update() #clear the screen ASAP
	...
	def partialrandom(threshold): #write random pixels to the screen with a weighted random chance
	    nonlocal lfsrstate
	    screen:ptr8 = thumby.display.display.buffer
	    pos = 0
	    bit = 0
	    lfsr = int(lfsrstate)
	    temp = 0
	...
	def oscillothread():
	    ...
	    lasttime = ticks_us[0] & 0x3fffffff #mask off upper bits to make room for signed comparisons
	    cmd = oscommand
	    while True:
	        cmd[0] = 0
