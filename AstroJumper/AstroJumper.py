...
while (1):
    ...
    def movePlatformsDown(sprites, stars, height):
        global lastMovement
        for star in stars:
            star.y = star.y - (height - lastMovement) * \
                (random.uniform(.3, .5))
        ...
        for sprite in sprites:
            ...
            if sprite.y >= thumby.display.height + 5:
                ...
                if (time.ticks_ms() % 2 == 0):
                    newPlat.x = random.randint(
                        thumby.display.width - int(thumby.display.width / 2), thumby.display.width - 8)
                else:
                    newPlat.x = random.randint(
                        0, thumby.display.width - int(thumby.display.width / 2))
                newPlat.y = 2
    ...
    while (1):
        ...
        thumby.display.drawText(
            str(totalScore), 0, thumby.display.height - 7, 1)
        thumby.display.drawText(
            "PWR" + str(powerUps), thumby.display.width - 25, thumby.display.height - 7, 1)
        thumby.display.update()
    ...
    while (1):
        ...
        thumby.display.drawText(str(totalScore), offset,
                                thumby.display.height - 25, 1)
        ...
        if (thumby.saveData.hasItem("highscore")):
            ...
            if totalScore > highScore:
                ...
                thumby.display.drawText(
                    "New Record!", 5, thumby.display.height - 7, 1)
            else:
                record = "Record: " + str(highScore)
                thumby.display.drawText(
                    record, 0, thumby.display.height - 7, 1)
        thumby.display.update()
        if (thumby.inputPressed() == True):
            break
