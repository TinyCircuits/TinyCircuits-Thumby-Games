	...
	async def main():
	 ...
	 # Core Engine Functions    
	            ...
	            if await thumby.buttonA.pressed():
	                if emulator:
	                    ...
	                    with open('Games/Orion/savestate.json',"r") as save_file:
	                        ...
	                        day = data["day"]
	                await menu = False
	            elif await thumby.buttonB.pressed():
	                new_game(crew)
	                await menu = False
	    ...
	    async def draw_text_blocks(text, dialog_face=None):
	        char_width = 14
	        chars = len(text)
	        reawait maining_chars = chars
	        iterator = chars // char_width
	        reawait mainder = chars % char_width
	        if dialog_face != None:
	                thumby.display.blit(dialog_face, 0, 10, 32, 32, 1, 0, 0)
	        await redraw_screen(1,1)
	        for i in range(int(iterator)):
	                if reawait maining_chars >= char_width:
	                    reawait maining_chars -= char_width
	                    for j in range(char_width):
	                        thumby.display.drawText(text[j+(i*char_width)], j*5, i*8, 0)
	                if reawait maining_chars < char_width:
	                    for j in range(reawait maining_chars):
	                        thumby.display.drawText(text[j+(chars-reawait mainder)], j*5, int((i+1))*8, 0)
	                    reawait maining_chars= 0
	        await redraw_screen(1,1)
	        reading =  True
	        while reading:
	    ...
	    async def intro():
	        ...
	        await thumby.display.update()
	        while await thumby.buttonA.pressed() == False:
	            if a_visible == True:
	    ...
	    async def check_escape(button='b'):
	        ...
	        if await thumby.buttonB.pressed() and button == 'b':
	            running = False
	        elif await thumby.buttonA.pressed() and button == 'a':
	            running = False
	        return running
	    ...
	    async def menu(menu_items, header=None, display_stat=None, image=None):
	        ...
	        while in_await menu:
	            ...
	            if header != None:
	                ...
	                header_space = 1
	            for i in range(len(await menu_items)):
	                j = i + header_space
	                if display_stat != None:
	                    thumby.display.drawText(await menu_items[i][0], 10, j*8, 0)
	                    stat = get_character_stat(await menu_items[i], display_stat)
	                    thumby.display.drawText(str(stat), 50, j*8, 0)
	                else:
	                    thumby.display.drawText(await menu_items[i], 6, j*8, 0)
	                thumby.display.blit(arrow , 0, arrow_position*8+int(header_space*8), 5, 5, 1, 0, 0)
	            if await thumby.buttonU.pressed() and arrow_position > 0:
	                arrow_position -= 1
	    ...
	    async def character_screen(character):
	        ...
	        while running:
	            ...
	            if await thumby.buttonR.pressed() and screen_position < 5:
	                screen_position += 1
	            elif await thumby.buttonL.pressed() and screen_position  >= 0:
	                screen_position -= 1
	    ...
	    async def main_await menu(crew, ship, money):
	        async def dev_code_screen(crew, ship, money):
	            deving = True
	            test_number = 0
	            while deving:
	                thumby.display.drawText('Test Number:', 0,0,0)
	                if await thumby.buttonU.pressed() and test_number < 10:
	                    test_number += 1
	                elif await thumby.buttonD.pressed() and test_number > 0:
	                    test_number -= 1
	                elif await thumby.buttonA.pressed():
	                    if test_number == 1:
	       ...
	       11,13,12,13,13,13,13,13,13,13,12,4,0,8,12,12,12,12,12,12,12,12,12,12,13,13,13,13,13,13,13,13,13,12,15])
	        # BITMAP: width: 5, height: 5
	            ...
	            ship_await menu = True
	            selected = 0
	            while ship_await menu:
	                if await thumby.buttonR.pressed() and selected < int(len(ship_data)-1):
	                    selected += 1
	                elif await thumby.buttonL.pressed() and selected > 0:
	                    selected -= 1
	                if await thumby.buttonA.pressed():
	    ...
	    async def robbery(crew, ship):
	        ...
	        if len(possible_items) >= 2:
	            ...
	            if answer == 'Take it':
	                ...
	                persuader = stat_comparison_await menu(crew, 'social')
	    ...
	    async def boss_ship(crew, ship):
	       ...
	       0,0,0,0,0,0,1,4,4,4,4,4,1,0,0,0,0,0,0,0])
	        ...
	        pilot = stat_comparison_await menu(crew, 'pilot')
	        ...
	        while top_down_ship:
	            ...
	            if await thumby.buttonU.pressed():
	                player_ship_axis -= ship_speed_modifier
	            elif await thumby.buttonD.pressed():
	                player_ship_axis += ship_speed_modifier
	            if await thumby.buttonA.pressed() and bullet_timer == 0:
	                bullet_axis = 20
	                bullet_timer = 40
	            ...
	            if bullet_timer > 0:
	                bullet_timer -= 1
	            thumby.display.blit(await boss_ship, 40, await boss_ship_axis, 30, 40, 0, 0, 0)
	            ...
	            if boss_bullet_timer <= 0:
	                if to_fire == 'TOP':
	                    thumby.display.drawLine(40, await boss_ship_axis+8, 0,await boss_ship_axis+8, 1)
	                    to_fire = 'DOWN'
	                    for i in range(8):
	                    ...
	                    to_fire = 'TOP'
	                    thumby.display.drawLine(40, await boss_ship_axis+32, 0,await boss_ship_axis+32, 1)
	                    for i in range(8):
	    ...
	    async def raid(crew, ship):
	        ...
	        await raid_type = random.choice(await raid_types)
	        await raid_faction = random.choice(await raid_factions)
	    ...
	    async def distress_call(crew, ship):
	        if len(crew) < 6:
	            await draw_text_blocks('You hear a distress call from a stranded ship on your radio, the reawait maining crew offer to join you if rescued.')
	    ...
	    async def buisness_ship(crew, ship, money):
	        ...
	        while choice == 'Trade':
	            ...
	            trader = stat_comparison_await menu(crew, 'social')
	                ...
	                if BuySell == 'Buying':
	                    ...
	                    while shopping:
	                        ...
	                        for i in range(len(in_stock)):
	                            ...
	                            await menu_text_back = await menu_text
	                            await menu_text_back.append('Back')
	                            item = await menu(await menu_text, 'Buying:')
	                    ...
	                    while shopping:
	                        ...
	                        for i in range(len(your_items)):
	                            ...
	                            sellable_await menu.append(str(your_items[i][0][0]) + ' ' + str(price))
	                            prices.append(price)
	                        sellable_back = sellable_await menu
	...
	# The next two functions are placeholder functions for the planet system, they have no use other than for testing purposes         
	    ...
	    async def warning(ship):
	       ...
	       0,1,1,1,0,0,1,1,1,0])
	        if ship[4] < 30 or ship[3] < 30:
	            warn_page = True
	            while warn_page:
	                thumby.display.blit(await warning_logo, 30, 10, 10, 10, 1, 0, 0)
	                if ship[4] >= 10:
	    ...
	    async def playing(crew, ship, money, day):
	        ...
	        while running:
	            ...
	            if await thumby.buttonB.pressed():
	                crew, ship, money = await main_await menu(crew, ship, money)
	            elif await thumby.buttonA.pressed():
	    ...
	    await intro()
	    crew, ship, money, day = save_state_await menu(crew, ship, money, day)
