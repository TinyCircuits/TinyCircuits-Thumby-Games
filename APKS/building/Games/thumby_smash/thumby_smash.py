	...
	async def singleplayer_battle(char, enemy_char, config):
	    async def character_move(name, jumpheight, attack, health, speed, weight, player_type, state, x, y, direction, a_x, a_y, a_jumpheight, a_attack, a_health, a_speed, a_weight, cooldown, knockback_state, data, config):
	        ...
	        if player_type == 'human':
	            if await thumby.buttonB.pressed() and await thumby.buttonU.pressed():
	                BU_Pressed = True
	            elif await thumby.buttonA.pressed() and await thumby.buttonR.pressed():
	                AR_Pressed = True
	            elif await thumby.buttonA.pressed() and await thumby.buttonU.pressed():
	                AU_Pressed = True
	            elif await thumby.buttonA.pressed():
	                A_Pressed = True
	            elif await thumby.buttonB.pressed():
	                B_Pressed = True
	            elif await thumby.buttonR.pressed():
	                R_Pressed = True
	            elif await thumby.buttonL.pressed():
	                L_Pressed = True
	            elif await thumby.buttonU.pressed():
	                U_Pressed = True
	...
	async def main_menu():
	     ...
	     async def config_menu():
	         ...
	         while configmenu == True:
	             if await thumby.buttonD.pressed() and arrow_location < 2:
	                 arrow_location += 1
	             elif await thumby.buttonU.pressed() and arrow_location > 0:
	                 arrow_location -= 1
	             thumby.display.fill(1)
	             ...
	             time.sleep(0.1)
	             if await thumby.buttonA.pressed() and arrow_location == 0:
	                 ai_await config_menu = True
	                 while ai_await config_menu == True:
	                     if await thumby.buttonD.pressed() and arrow_location == 0:
	                        arrow_location += 1
	                     elif await thumby.buttonU.pressed() and arrow_location == 1:
	                        arrow_location -= 1
	                     elif await thumby.buttonB.pressed():
	                         ai_await config_menu = False
	                     if arrow_location == 0:
	                         thumby.display.blit(menu_arrow, 0, 0, 5, 5, 1, 0, 0)
	                         if await thumby.buttonR.pressed() and ai_stupidity < 20:
	                            ai_stupidity += 1
	                         elif await thumby.buttonL.pressed() and ai_stupidity > 0:
	                            ai_stupidity -= 1
	                     elif arrow_location == 1:
	                         thumby.display.blit(menu_arrow, 0, 25, 5, 5, 1, 0, 0)
	                         if await thumby.buttonR.pressed() and ai_mode < 1:
	                            ai_mode += 1
	                         elif await thumby.buttonL.pressed() and ai_mode > 0:
	                            ai_mode -= 1
	                     ...
	                     await thumby.display.update()
	                     thumby.display.fill(1)
	             elif await thumby.buttonA.pressed() and arrow_location == 1:
	                 gameplay_await config_menu = True
	                 while gameplay_await config_menu == True:
	                     ...
	                     if arrow_location == 0:
	                         ...
	                         if await thumby.buttonR.pressed() and game_pace < 10:
	                            game_pace += 1
	                         elif await thumby.buttonL.pressed() and game_pace > 1:
	                            game_pace -= 1
	                     elif arrow_location == 1:
	                         thumby.display.blit(menu_arrow, 0, 30, 5, 5, 1, 0, 0)
	                         if await thumby.buttonR.pressed() and wall_int < 2:
	                            wall_int += 1
	                         elif await thumby.buttonL.pressed() and wall_int > 0:
	                            wall_int -= 1
	                     if wall_int == 0:
	                         wall_behavior = 'Block'
	                     elif wall_int == 1:
	                     ...
	                     if await thumby.buttonB.pressed():
	                             gameplay_await config_menu = False
	                     time.sleep(0.1)
	                     await thumby.display.update()
	                     thumby.display.fill(1)
	                     
	             elif await thumby.buttonA.pressed() and arrow_location == 2 or await thumby.buttonB.pressed():
	                 configmenu = False
	                 menu = True
	...
	async def character_select(config):
	    ...
	    while select_screen == True:
	       ...
	       # BITMAP: width: 16, height: 16
	        ...
	        if await thumby.buttonR.pressed() and selected < int(len(chars)-1) :
	            selected += 1
