	...
	class CPU:
	    ...
	    def clockTick(self, t):
	        ...
	        # few CPU cycles)
	        self.await rendering = not self.await rendering
	        if self.display.dirty and self.await rendering:
	            thumbyinterface.await render(self.display.width, self.display.height, self.display.frameBuffers)
