...
async def main():
	...
	class player():
	    ...
	    async def user_move(self):
	        ...
	        if await thumby.buttonU.pressed():
	            if self.charSprite.x > 29 and \
	                ...
	                self.facing = 0
	        if await thumby.buttonD.pressed():
	            if self.charSprite.x > 29 and \
	                    self.charSprite.x < 38 and \
	    ...
	    def user_attack(self):
	        ...
	        elif self.attacking > 0 and not await thumby.buttonA.pressed():
	            self.attacking -= 1
	            return 0,0
	        else: return 0,0
	    ...
	    if status == 0: 
	        ...
	        if p1.room == stairs_loc:
	            ...
	            if px < 42 and px > 25 and py < 27 and py > 8 and shopCooldown == 0:
	                ...
	                atk = p1.attack
	                s = shop()
	                gold,life,atk = s.await shopping_loop(gold,life,atk)
	                p1.gold = gold
	                p1.maxlife = life
	                p1.attack = atk
	                p1.life = p1.maxlife
