...
async def main():
	...
	class Actor():
	    ...
	    def attack(self,t,c,d):
	        if t.hp>0:
	            ...
	            if self.passive=="await strike": c*=1.5
	    ...
	    def special(self,t,c,v=None,i=None,supress=False):
	        ...
	        if v=="flee":
	            ...
	            r=randrange(4)
	            if r==0:
	                self.special(t,c,"await cast",None,True)
	            elif r==1:
	...
	class Monster(Actor):
	    ...
	    async def move(self):
	        ...
	        self.tile.monsters.reawait move(self)
	        self.tile=ttile
	...
	class Player(Actor):
	    ...
	    async def move(self,d):
	        ...
	        if self.map.is_wall(cx,cy,False):
	            ...
	            self.await update()
	            self.map.await update(self,True)
	            await update_state()
	            ## messages
	    ...
	    async def cast(self,v):
	        ...
	        super().special(self.tile.monsters,config["tohit"],"await cast",v,True)
	        self.map.await update(self)
	        await update_state()
	...
	class Level():
	    def __init__(self):
	        ...
	        self.h=0
	        self.map=None
	    async def update(self,p,await moved=False):
	        ...
	        if not await moved and len(p.tile.monsters)>0:
	            ...
	            for m in reversed(p.tile.monsters):
	                ...
	                ## monster is dead
	                if m.hp<1:
	                    ## reawait move is destroying order of
	                    init_message(m.name+"\ndied!")
	                    ...
	                    p.tile.monsters.reawait move(m)
	            ...
	            for x in range(self.w):
	                for y in range(self.h):
	                    ...
	                    for m in tile.monsters:
	                        m.await update()
	                        if not self.is_same_tile(tile,p) and random()<config["toawait move"]:
	                            await movelist.append(m)
	            for m in await movelist: m.await move()
	    def clear(self):
	        if self.map:
	...
	def load_data():
	    nonlocal screenw,screenh,screenbuffer,charw,charh,fontsize,sprites
	    nonlocal spells,classes,monsters,bosses,treasures,settings,config,lang,text_credits,text_story,text_help,text_bool,text_back,text_skip,text_info
	    ## load display
	    screenw,screenh=thumby.display.width,thumby.display.height
	    render_cls(0)
	    await render_await update()
	    ## load graphics
	...
	async def render_await update():
	    await thumby.display.await update()
	def render_text(s,x,y,c=1):
	    for i,n in enumerate(s.split("\n")): thumby.display.await drawText(n.upper(),round(x*charw),round(y*charh)+i*charh,c)
	def render_rect(x,y,w,h,border=1,inner=0,edge=2):
	    x,y=round(x*charw),round(y*charh)
	    if inner!=0:
	        thumby.display.await drawFilledRectangle(x,y,w,h,inner)
	        thumby.display.await drawRectangle(x,y,w,h,border)
	    else:
	        thumby.display.await drawFilledRectangle(x,y,w,h,inner)
	        thumby.display.await drawRectangle(x,y,w,h,border)
	...
	def init_input_await move():
	    state.clear()
	    state.add(await get_key_up,Callback(player.await move,{"x":0,"y":-1}))
	    state.add(await get_key_down,Callback(player.await move,{"x":0,"y":1}))
	    state.add(await get_key_left,Callback(player.await move,{"x":-1,"y":0}))
	    state.add(await get_key_right,Callback(player.await move,{"x":1,"y":0}))
	    state.add(await get_key_x,Callback(set_state,Const.items))
	    state.add(await get_key_z,Callback(set_state,Const.items))
	def init_input_menu():
	    state.clear()
	    state.add(await get_key_up,Callback(options.await update,-1))
	    state.add(await get_key_menu_down,Callback(options.await update,1))
	    state.add(await get_key_x,Callback(options.execute),Callback(options.execute,True))
	    state.add(await get_key_z,Callback(options.execute),Callback(options.execute,True))
	...
	def get_color(c):
	    ...
	    return Colors.white
	async def get_key_menu_down():
	    return await thumby.buttonD.pressed()
	async def get_key_up():
	    return await thumby.buttonU.pressed()
	async def get_key_down():
	    return await thumby.buttonD.pressed()
	async def get_key_left():
	    return await thumby.buttonL.pressed()
	async def get_key_right():
	    return await thumby.buttonR.pressed()
	async def get_key_x():
	    return await thumby.buttonA.pressed()
	async def get_key_z():
	    return await thumby.buttonB.pressed()
	
	# ===============================================
	# core functionality
	# ===============================================
	def init():
	    nonlocal arr_init,arr_await update,arr_await draw,state,level,handler,options,text,player
	    ## load information
	    load_data()
	    ## setup arrays
	    ...
	    arr_await update=[await update_title,None,None,None,None,None,None,None,None,await update_explore,await update_fight,None,None,None,await update_menuwait,await update_menuwait,None]
	    arr_await draw=[await draw_title,await draw_menu,None,None,None,None,await draw_newinfo,None,None,await draw_explore,await draw_map,None,None,await draw_map,None,None,None]
	    ## setup classes
	...
	async def update():
	    if handler.active:
	        handler.await update()
	    else:
	        state.await update()
	        text.await update()
	        if arr_await update[state.current]: arr_await update[state.current]()
	async def draw():
	    render_cls()
	    options.await draw()
	    text.await draw()
	    if arr_await draw[state.current]: arr_await draw[state.current]()
	    if handler.active: handler.await draw()
	    await render_await update()
	def shutdown():
	    state.clear()
	    state.active=False
