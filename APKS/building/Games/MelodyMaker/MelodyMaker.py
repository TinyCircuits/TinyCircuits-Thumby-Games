...
async def main():
	...
	while 1:
	    ...
	    #input section
	    if await thumby.buttonU.pressed() and not pressed:
	        noteSpots[curNote].inc()
	        noteSpots[curNote].play(500)
	        pressed = True
	    elif await thumby.buttonD.pressed() and not pressed:
	        noteSpots[curNote].dec()
	        noteSpots[curNote].play(500)
	        pressed = True
	    elif await thumby.buttonL.pressed() and not pressed:
	        if curNote > 0:
	            curNote -= 1
	        pressed = True
	    elif await thumby.buttonR.pressed() and not pressed:
	        if curNote < len(noteSpots) - 1:
	            curNote += 1
	        pressed = True
	    elif await thumby.buttonA.pressed() and not pressed:
	        saveSequence(noteSpots)
	        await playMelody(noteSpots, curNote)
	        pressed = True
	    elif await thumby.buttonB.pressed() and not pressed:
	        noteSpots[curNote].toggleOctave()
	        noteSpots[curNote].play(500)
	        pressed = True
	    elif not await thumby.buttonD.pressed() and not await thumby.buttonU.pressed() and not await thumby.buttonL.pressed() and not await thumby.buttonR.pressed() and not await thumby.buttonA.pressed() and not await thumby.buttonB.pressed():
	        pressed = False
