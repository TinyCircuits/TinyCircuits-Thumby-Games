	...
	def configure(types=None, corecount=7): #set up state machines
	    nonlocal synthcore, channels, tickfreq, channelcount, config
	...
	def stop(mixer=True, chan=True, song=True):
	    nonlocal channelcount, playing, notes
	    if song:
	        playing = False
	        timer.deinit()
	...
	#
	def audiotick(dummy):
	    nonlocal playing, eventstart, notes, stream
	    stop = True
	...
	def play(song, ins={}, autoenable=True, loop=False): #song events, instruments, whether or not to automatically set/change the channel count
	    nonlocal stream, eventstart, playing, instruments, ilist, setenabled, autoreset
