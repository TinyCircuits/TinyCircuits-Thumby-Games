...
async def main():
	...
	def drawCave(caveX):
	    ...
	    nonlocal obstacleLastSpawnTime
	    nonlocal obstaclePos
	...
	async def showEndScreen():
	    ...
	    while await thumby.buttonA.pressed() or await thumby.buttonB.pressed():
	        pass
	...
	def handleInput():
	    ...
	    if await thumby.buttonR.pressed() and heliSprite.x + 23 < thumby.display.width:
	        heliSprite.x = heliSprite.x + 1
	    elif await thumby.buttonL.pressed() and heliSprite.x -1 > 0:
	        heliSprite.x = heliSprite.x - 1
	    
	    # Spawn a missile
	    nonlocal missileFired
	    if await thumby.buttonD.pressed() and missileFired == False:
	        missileSprite.x = heliSprite.x+11
	        missileSprite.y = heliSprite.y+2
	        missileFired = True
	
	
	async def checkCollision():
