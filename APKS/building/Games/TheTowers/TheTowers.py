...
async def main():
	...
	#=======================================================================================
	def drawSprite( inspr, x, y, width, height, key=-1, frame = 0, masked = False):
	    sprite = []
	    if(nonlocals().get('ImageFile') is not None and type(inspr)==ImageFile):
	        if(inspr.masked):
	            masked = True
	...
	def WinScreenShow():
	    ...
	    if(thumby.actionJustPressed()):
	        HighScores.await LoadScores()
	        HighScores.NewScore(Moves.moves)
	        SaveGame.deleteSave()
	        return 3
	    return 10
	    
	def MainMenuShow(menu,state,choose):
	    #Main Menu Section
	    count = 0
	    if (menu == 0):
	...
	class MovesCounter:
	    def __init__(self,_init_x,_init_y,_moves,_show:bool):
	        self.x = _init_x
	        self.y = _init_y
	...
	class Disk:
	    def __init__(self,_init_x,_init_y,_top:bool,_bottom:bool,_size):
	        self.x = _init_x
	        self.y = _init_y
	        self.top = _top
	        self.bottom = _bottom
	    ...
	    def setTargets(self,target_x,target_y):
	...
	class Cursor:
	    def __init__(self,_init_x,_init_y,_init_off):
	        self.x = _init_x
	        self.y = _init_y
