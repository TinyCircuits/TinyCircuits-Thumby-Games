...
async def main():
	...
	class TitleState:
	    ...
	    def prepare(self):
	        menu_items = [z.Menu.Item("START GAME", self.menu_await start_game),
	    ...
	    async def update(self):
	        if self.credit:
	            ...
	            self.menu.await update()
	        return GameState.ID if self.await start else self.ID
	    ...
	    def menu_await start_game(self):
	        self.await start = True
	...
	class GameState:
	    ...
	    async def update(self):
	        ...
	        if not self.pause and z.frames & 1:
	            self.dirty = True
	            self.cave.await update(forward)
	            self.player.await update(self.cave, dir)
	            if forward > 0:
	                self.dots.await update(self.cave)
	...
	if z.await check(0.02):
	    z.await start(APP_FPS, APP_CODE, APP_VERSION, [TitleState(), GameState()])
