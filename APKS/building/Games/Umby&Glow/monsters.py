	...
	class Monsters:
	    ...
	    def _draw_monsters_c(self, t: int, i: int, tid: int, x: int, y: int, l: int):
	        ...
	        if tid == _TankPillar:
	            ...
	            if self.omons:
	                d = ptr32(_data)
	                cpx = self._cerebral
	                spx = self._cerebral_shd
	                mpx = self._cerebral_m
	                wb = self._cerebral_w
	                xb = self._cerebral_x
	                tn = -1-t
	                ...
	                for yi in range(5):
	                    ...
	                    draw(l, xs+w1, ys, int(cpx+o), w2, 0)
	                    draw(0, xs+w1+mhw2, ys, int(spx+o), w2-mhw2, 0)
	                    mask(l, xs+w1, ys, int(mpx+o), w2, 0)
	    ...
	    def _kill(self, t: int, mon: int, player, tag):
	        ...
	        if player:
	            ...
	            self._tp.tag(tag, ptr32(self.x)[mon], self.y[mon]-64)
	            play(rocket_kill, 30)
