	...
	thumby2.await requireMinThumbyVersion("1.6")
	...
	def menuChoice(options,row,rows,selOption=0):
	    ...
	    while True:
	        ...
	        time.sleep_ms(50)
	        thumby2.buttons.aawait wait update()
	...
	class FBuf:
	    ...
	    async def __init__(self,size,buf=None):
	        if not buf:
	...
	class Canvas:
	    ...
	    async def __init__(self,size,buf=None,name=None):
	        self.name=name
	        self.size=size
	        self.width=size[0]
	        self.height=size[1]
	    ...
	    def _redrawZoomBufY(self,y0,y1):
	        pixelSize=self.pixelSize
	        sy=-self.scrollY
	        for y in range(self.height):
	    ...
	    def _redrawZoomBufX(self,x0,x1):
	        pixelSize=self.pixelSize
	        sx=-self.scrollX
	        for x in range(self.width):
