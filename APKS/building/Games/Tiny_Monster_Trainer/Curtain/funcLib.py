	...
	async def thingAquired(word1, word2, itemName, word4 ="", setSleep=1, skipUpdate=0, skipFill=0):
	    if skipFill == 0:
	...
	async def battleStartAnimation(color):
	...
	async def popItOff(theListofObjs, word):
	    thumby.display.fill(0)
	    #gc.collect()
	...
	def buttonInput(selectPos):                
	    ...
	    while(thumby.dpadJustPressed() == False and thumby.actionPressed == False):
	        pass
	    if(await thumby.buttonU.pressed() == True):
	        while(await thumby.buttonU.pressed() == True): 
	            pass
	        selectionBoxPos = selectionBoxPos - 1
	    elif(await thumby.buttonD.pressed() == True):
	        while(await thumby.buttonD.pressed() == True): 
	            pass
	        if (selectionBoxPos <= 26):
	            selectionBoxPos = selectionBoxPos + 1
	    elif(await thumby.buttonL.pressed() == True):
	        while(await thumby.buttonL.pressed() == True): 
	            pass
	        selectionBoxPos = 29
	    elif(await thumby.buttonR.pressed() == True):
	        while(await thumby.buttonR.pressed() == True): 
	            pass
	        selectionBoxPos = 28
	    elif(await thumby.buttonA.pressed() == True):
	        while(await thumby.buttonA.pressed() == True):
	            pass
	        selectionBoxPos = 31
	    elif(await thumby.buttonB.pressed() == True):
	...
	async def tameMon(playerInfo, npcMon):
	    gc.collect()
	    newMon = Monster()
	    ...
	    if len(playerInfo.friends) > playerInfo.playerBlock['friendMax']:
	            await popItOff(playerInfo.friends, "monsters, please let one go!")
	...
	async def showMonInfo(playerInfo, startOfgameCheck=0, combatCheck=0):
	    left = 1
	    right = -1
	    down = -1
	    up = -1
	    ...
	    while(goBack != 1): 
	        ...
	        if currentSelect == -2: 
	            ...
	            await thingAquired(monsterListInfo[x].statBlock['given_name'], "is a", monsterListInfo[x].statBlock['name'], "", 0, 1)
	            drawArrows(left, right, down, up)
	            ...
	            while(monsterListInfo[x].statBlock[monsterListInfo[x].keyList[currentSelect]] == ""):
	                if tempSelect2 < currentSelect:
	                    ...
	                    currentSelect = currentSelect - 1
	            await thingAquired(monsterListInfo[x].statBlock['given_name'] + "'s",
	        ...
	        if currentSelect == 31 and combatCheck != 1:
	            if playerInfo.friends[0] != playerInfo.friends[x] or startOfgameCheck == 1:
	                if playerInfo.friends[x].statBlock['currentHealth'] == 0:
	                    await thingAquired(monsterListInfo[x].statBlock['given_name'], "does not", "have enough", "HP to fight!", 2)
	                ...
	                if combatCheck != 2: #need to while switching in battle if selected mon has HPs
	                    await thingAquired(monsterListInfo[0].statBlock['given_name'], "is now", "your active", "monster!", 2)
	                x = 0
	                currentSelect = -2
