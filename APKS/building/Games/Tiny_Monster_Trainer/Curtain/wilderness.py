	...
	from funcLib import await thingAquired, await battleStartAnimation, printMon, drawArrows, showOptions, await popItOff, buttonInput, noDupAtk, await giveName, await tameMon, switchActiveMon, save, await showMonInfo
	#import micropython
	...
	async def attackAnimation(playerBod, nmeBod, attackIsPlayer, missFlag, amountOfDmg, playerHP, nmeHP, atkTxt, attackType = ""):
	...
	async def afterAttackSelect(attackingMon, atkChoice, defMon, playerTrainLevel, npcTrainLevel, attackIsPlayer):
	    scrollText = ""
	    ...
	    if amntOfDmg >= 1:
	        if attackIsPlayer == 1:
	            await attackAnimation(attackingMon.bodyBlock, defMon.bodyBlock, attackIsPlayer, 0, amntOfDmg, attackingMon.statBlock['currentHealth'], hpBeforeDmg, attackText, attackingMon.attackList[atkChoice].moveElementType)
	            scrollText = (attackingMon.statBlock['given_name'] + " did " + str(amntOfDmg) + " points of damage!")
	        else:
	            await attackAnimation(defMon.bodyBlock, attackingMon.bodyBlock, attackIsPlayer, 0, amntOfDmg, hpBeforeDmg, attackingMon.statBlock['currentHealth'], attackText, attackingMon.attackList[atkChoice].moveElementType)
	    else:
	        if attackIsPlayer == 1:
	            await attackAnimation(attackingMon.bodyBlock, defMon.bodyBlock, attackIsPlayer, 1, amntOfDmg, attackingMon.statBlock['currentHealth'], hpBeforeDmg, attackText)
	            scrollText = (attackingMon.statBlock['given_name'] + "'s " + attackingMon.attackList[atkChoice].name + " attack missed!" )
	        else: 
	            await attackAnimation(defMon.bodyBlock, attackingMon.bodyBlock, attackIsPlayer, 1, amntOfDmg, hpBeforeDmg, attackingMon.statBlock['currentHealth'], attackText)
	    return scrollText
	
	
	async def attackOptionMenu(monInfo):  
	    currentSelect = 1
	...
	async def battleScreen(playerMon, nmeMon, playerTrainLevel, npcTrainLevel):
	    ...
	    while((playerMon.statBlock['currentHealth'] >= 1) and (nmeMon.statBlock['currentHealth'] >= 1)):
	        ...
	        if currentSelect == 31: 
	            ...
	            if options[currentSelect] == "Atk": 
	                selectCheck = await attackOptionMenu(playerMon.attackList)
	                if selectCheck < 30:
	                    if playerMon.attackList[selectCheck].currentUses <= 0:
	                        ...
	                        await thingAquired(playerMon.statBlock['given_name'], "is out of", "stamina", "HP lost", 2)
	                    ...
	                    if (playerMon.statBlock['Agility'] + playerTrainLevel + agileTie) >= (nmeMon.statBlock['Agility'] + npcTrainLevel):
	                        myScroller = TextForScroller(await afterAttackSelect(playerMon, selectCheck, nmeMon, playerTrainLevel, npcTrainLevel, 1))
	                        if npcMon.statBlock['currentHealth'] <= 0:
	                            ...
	                            return 1 
	                        junk = await afterAttackSelect(nmeMon, (len(nmeMon.attackList) -1), playerMon, npcTrainLevel, playerTrainLevel, 0) 
	                        del junk
	                    else:
	                        junk = await afterAttackSelect(nmeMon, (len(nmeMon.attackList) -1), playerMon, npcTrainLevel, playerTrainLevel, 0)
	                        del junk
	                ...
	                await showMonInfo(tempPlayer, 0 , 1)
	                del tempPlayer
	...
	async def autoSwitchMon(playerInfo):
	    if playerInfo.friends[0].statBlock['currentHealth'] < 1:
	        ...
	        if playerInfo.friends[0].statBlock['currentHealth'] > 0:
	            await thingAquired(playerInfo.friends[0].statBlock['given_name'], "is now", "your active", "monster!", 2)
	
	
	async def playerInformation(playerInfo):
	    thumby.display.fill(0)
	...
	async def displayItems(playerInfo):
	    thumby.display.fill(0)
	    curSelect = 0
	    ...
	    if x > 0:
	        while curSelect < 11:
	            ...
	            if curSelect ==  31:
	                ...
	                await thingAquired(playerInfo.playerBlock['name'],
	...
	async def trainActiveMon(myMonStats, monsterBody):
	    #gc.collect()
	    thumby.display.fill(0)
	    ...
	    while goBack != 1:
	        ...
	        if currentSelect < -6 + 2:
	            currentSelect = currentSelect + 6
	        thumby.display.fill(0)
	        await thingAquired(statNameList[currentSelect], statNumsList[currentSelect], trainingPointsTxt, myMonStats['given_name'], 0, 1)
	        drawArrows(1, 1, -1, -1)
	        tempSelect = currentSelect
	        ...
	        if currentSelect == 28 or currentSelect == 29:
	            ...
	            if myMonStats['trainingPoints'] > 0:
	                if statNameList[currentSelect] == "Health" and myMonStats['Health'] < myMonStats['maxHealth']: 
	                    ...
	                    await trainAnimation(monsterBody)
	                    await thingAquired(myMonStats['given_name'], "trained", "their", "health!", 2) 
	                    ...
	                    await trainAnimation(monsterBody)
	                    await thingAquired(myMonStats['given_name'], "trained", "their", "agility!", 2)
	                    ...
	                    await trainAnimation(monsterBody)
	                    await thingAquired(myMonStats['given_name'], "trained", "their", "strength!", 2)
	                    ...
	                    await trainAnimation(monsterBody)
	                    await thingAquired(myMonStats['given_name'], "trained", "their", "endurance", 2)
	                    ...
	                    await trainAnimation(monsterBody)
	                    await thingAquired(myMonStats['given_name'], "practiced", "their", "mysticism", 2)
	                    ...
	                    await trainAnimation(monsterBody)
	                    await thingAquired(myMonStats['given_name'], "polished", "their", "tinfoil", 2)
	                else:
	                    await thingAquired("Stat is", "already", "maxed out", "", 2)
	                    myMonStats['trainingPoints'] = myMonStats['trainingPoints'] + 1
	                myMonStats['trainingPoints'] = myMonStats['trainingPoints'] - 1
	            else:
	                await thingAquired("Not", "Enough", "Trainer", "Points", 2)
	            await thumby.display.update()
	            goBack = 1
	        await thumby.display.update()
	
	
	async def optionScreen(playerInfo):
	    if(await thumby.buttonB.pressed() == True):
	        while(await thumby.buttonB.pressed() == True):
	            pass
	        thumby.display.fill(0)
	        #gc.collect()
	        curSelect = 1
	        ...
	        while cancelCheck != 1:
	            ...
	            if curSelect == 31:
	                ...
	                if optionList[curSelect] == optionList[0]:
	                    await playerInformation(playerInfo)
	                    ...
	                    while(goBack != 1):
	                        ...
	                        if curSelect == 31:
	                            ...
	                            if subOptionsFriends[curSelect] == subOptionsFriends[0]:
	                                await showMonInfo(playerInfo)
	                            if subOptionsFriends[curSelect] == subOptionsFriends[1]:
	                                await trainActiveMon(playerInfo.friends[0].statBlock, playerInfo.friends[0].bodyBlock)
	                            if subOptionsFriends[curSelect] == subOptionsFriends[2]:
	                                await trainAnAttackMove(playerInfo.friends[0].attackList, playerInfo.friends[0].statBlock, playerInfo.friends[0].keyList)
	                                while len(playerInfo.friends[0].attackList) > 6:
	                                    await popItOff(playerInfo.friends[0].attackList, "moves! Please forget one!")
	                            if subOptionsFriends[curSelect] == subOptionsFriends[3]:
	                                playerInfo.friends[0].statBlock['given_name'] = await giveName(playerInfo.friends[0].statBlock['given_name'])
	                            if subOptionsFriends[curSelect] == subOptionsFriends[4]:
	                                await mutateMon(playerInfo.friends[0])
	...
	async def mutateMon(self):
	    ...
	    if self.statBlock['trainingPoints'] > 4:
	        ...
	        if self.mutateSeed[1] < 4:
	            ...
	            gc.collect()
	            #micropython.mem_info()
	            await mutateAnimation(tempBody, self.bodyBlock)
	            await thingAquired(self.statBlock['given_name'], "has", "mutated!", "", 2)
	        else:
	            await thingAquired(self.statBlock['given_name'], "is unable to", "mutate", "again", 2)
	    else:
	        howManyPoints = self.statBlock['trainingPoints']
	        await thingAquired(self.statBlock['given_name'], ("needs " + str(5 - howManyPoints) + " more"), "Training", "Points", 2)
	
	
	async def trainAnAttackMove(attackList, statBlock, keyList):
	        #gc.collect()
	        howManyTypes = 0
	        ...
	        if statBlock['trainingPoints'] >= 3:
	            ...
	            while(attackLearned != 1):
	                ...
	                if attacksKnown != checkKnownAttacks:
	                    attackLearned = 1
	                    await thingAquired(statBlock['given_name'], "learned", attackList[-1].name, "", 2)
	                    statBlock['trainingPoints'] = statBlock['trainingPoints'] - 3
	                    break
	                noAttacksToLearn = noAttacksToLearn  + 1
	                if noAttacksToLearn == 30:
	                    await thingAquired(statBlock['given_name'], "did not", "learn a", "new attack", 2)
	                    break
	        else:
	            howManyPoints = statBlock['trainingPoints'] 
	            await thingAquired(statBlock['given_name'], ("needs " + str(3 - howManyPoints) + " more"), "Training", "Points", 2)
	
	
	async def makeWorld(wSeed):
	    gc.collect()
	    await thingAquired("", "Generating", "World","", 0)
	    random.seed(wSeed) 
	    worldList = []
	...
	async def findAnItem(playerInv, maxItems):
	    #gc.collect()
	    newItem = Item("GenHeal", 1)
	    newItem.getItem()
	    playerInv.append(newItem)
	    await thingAquired("You", "found", newItem.name, "", 2)
	    if maxItems <= len(playerInv):
	        await popItOff(playerInv, "items! Please lose one.")
	...
	async def mutateAnimation(tempBody, monsterBody):
	    gc.collect()
	...
	async def trainAnimation(monsterBody):
	    f = open('/Games/Tiny_Monster_Trainer/Curtain/Other.ujson')
	    images = ujson.load(f)
	    t0 = 0
	    ct0 = time.ticks_ms()
	...
	async def loss(curMon):
	    ...
	    await thingAquired(curMon.statBlock['given_name'], "is", "Disheartened", "TP lost", 2)
	...
	myGuy = Player()
	myGuy = loadGame()
	world = await makeWorld(myGuy.playerBlock['worldSeed'])
	...
	while(1):
	    ...
	    while(battle != 1):
	        ...
	        if len(myGuy.friends) > myGuy.playerBlock['friendMax']:
	            await popItOff(myGuy.friends, "monsters, please let one go!")
	        thumby.display.fill(0)
	        ...
	        await optionScreen(myGuy)
	        await thumby.display.update()
	        if myGuy.currentPos == npcMonRoaming.currentPos:
	            npcMonRoaming.removeMonster()
	            battle = 1
	            await battleStartAnimation(1)
	    #try:
	    ...
	    while(battle == 1):
	        victory = 0
	        thumby.display.fill(0)
	        victory = await battleScreen(myGuy.friends[activeMon], battleMon, myGuy.playerBlock['trainerLevel'], npcTL)
	        await autoSwitchMon(myGuy)
	        if myGuy.friends[activeMon].statBlock['currentHealth'] == 0:
	            battle = 0
	            await loss(myGuy.friends[random.randint(0, len(myGuy.friends) - 1)])
	        if npcMon.statBlock['currentHealth'] == 0:
	            battle = 0
	        if victory == 2:
	                ...
	                for things in range(0, len(myGuy.inventory)):
	                    if myGuy.inventory[things-1].name == "Crystals":
	                        if (random.randint(0,20) + myGuy.inventory[things-1].bonus + random.randint(1, myGuy.playerBlock['trainerLevel'])) > 15: 
	                            await thingAquired(npcMon.statBlock['name'], "was", "Tamed!", "Yay!", 3)
	                            await tameMon(myGuy, npcMon)
	                            myGuy.friends[-1].statBlock['currentHealth'] = myGuy.friends[-1].statBlock['Health']
	                            myGuy.inventory.pop(things-1)
	                            battle = 0
	                            break
	                        else:
	                            await thingAquired("Crystal", "Used,", "Not", "Tamed", 2)
	                            myGuy.inventory.pop(things-1)
	                            break
	                    elif things == len(myGuy.inventory):
	                        await thingAquired("You don't", "have any", "Taming", "Crystals", 2) 
	            else:
	                await thingAquired("You don't", "have any", "Taming", "Crystals", 2)
	        if victory == 4:
	            await showMonInfo(myGuy, 0, 2)
	            victory = 0
	        await thumby.display.update()
	    await battleStartAnimation(0) 
	    if victory == 1:
	        myGuy.await levelUpCheck()
	        ...
	        if len(myGuy.inventory) < myGuy.maxHelditems:
	            ...
	            if randoNum > 2:
	                await findAnItem(myGuy.inventory, myGuy.maxHelditems)
