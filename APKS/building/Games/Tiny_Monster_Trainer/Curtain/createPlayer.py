	...
	from funcLib import await thingAquired, buttonInput, noDupAtk, await giveName, await tameMon, save, await showMonInfo
	
	    
	async def makeMonsterList(mSeed):
	    gc.collect()
	    random.seed(mSeed)
	    statDict = {}
	    bodyDict = {}
	    ...
	    for i in range (0 , 11): # number of mons = i * n  (Leave this at 11, so it can cycle through each type)
	        for n in range (0,2): # numberOfMons = 22  (game will work on the emulator at 33 monsters, but it will run out of memory as a fight starts in wilderness.py on a physical thumby)
	            gc.collect()
	            await thingAquired("", "Generating", "Monsters",str(monsterCount+1), 0)
	            newMon = Monster()
	    ...
	    try:
	        ...
	        myGuy = await makePlayer(monsterList[randomNum1], monsterList[randomNum2], monsterList[randomNum3], theWorldSeed)    
	...
	async def makePlayer(monster1, monster2, monster3, seed):
	    gc.collect()
	    currentSelect = 0
	    newPlayer = Player()
	    thumby.display.fill(0)
	    await thingAquired("Press A", "to give", "your", "name!", 0)
	    while(currentSelect != 31):
	        currentSelect = buttonInput(currentSelect)
	    currentSelect = 0
	    newPlayer.playerBlock['name'] = await giveName(newPlayer.playerBlock['name'])
	    thumby.display.fill(0)
	    await thingAquired("Press A", "to pick", "your", "Monster!", 0)
	    while(currentSelect != 31):
	        currentSelect = buttonInput(currentSelect)
	    currentSelect = 0
	...
	try:
	    ...
	    monsterList = await makeMonsterList(theWorldSeed) 
