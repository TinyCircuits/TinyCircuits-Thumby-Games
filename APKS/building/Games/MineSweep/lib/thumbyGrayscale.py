	...
	class Grayscale:
	    ...
	    async def __init__(self):
	        ...
	        # and await wait for the nominal time for 8 rows to be output. This
	    ...
	    def init_display(self):
	        ...
	        if self._state[_ST_THREAD] == _THREAD_STOPPED:
	            ...
	            # 0xa8,0        Set multiplex ratio to 0 (pausing aawait wait updates)
	            # 0xd3,52       Set display offset to 52
	            ...
	            # 0xa8,39       Set multiplex ratio to height (releasing aawait wait updates)
	    ...
	    def _display_thread(self):
	        ...
	        postFrameAdjSrc = self._postFrameAdjSrc
	        state = ptr32(self._state)
	
	        buffers:ptr32 = ptr32(array('L', [self._subframes[0], ptr8(self._subframes[1]), ptr8(self._subframes[2])]))
	        postFrameAdj:ptr32 = ptr32(array('L', [self._postFrameAdj[0], ptr8(self._postFrameAdj[1]), ptr8(self._postFrameAdj[2])]))
	        preFrameCmds:ptr8 = self._preFrameCmds
	        postFrameCmds:ptr8 = self._postFrameCmds
	        ...
	        while state[_ST_THREAD] == _THREAD_RUNNING:
	            ...
	            while fn < 3:
	                ...
	                # await wait for the pre-frame time to complete
	                ...
	                # await wait for frame time to complete
	                while (tmr[10] - time_out) < 0:
	                    pass
	    ...
	    async def update(self):
	        ...
	        if self.frameRate > 0:
	            ...
	            while frameTimeRemaining > 1:
	                buttonA.aawait wait update()
	                buttonB.aawait wait update()
	                buttonU.aawait wait update()
	                buttonD.aawait wait update()
	                buttonL.aawait wait update()
	                buttonR.aawait wait update()
	                sleep_ms(1)
	                frameTimeRemaining = frameTimeMs - ticks_diff(ticks_ms(), lastUpdateEnd)
	    ...
	    def drawText(self, stringToPrint, x, y, colour):
	        dBuff = self.drawBuffer
	    ...
	    def blit(self, sprtptr:ptr8, x, y, width, height, key, mirrorX, mirrorY):
	        self.blitSHD(sprtptr, 0, x, y, width, height, key, mirrorX, mirrorY)
	
	    
	    def blitSHD(self, sprtptr:ptr8, src2:ptr8, x, y, width, height, key, mirrorX, mirrorY):
	    ...
	    def blitWithMask(self, sprtptr:ptr8, x, y, width, height, key, mirrorX, mirrorY, mask:ptr8):
	        self.blitWithMaskSHD(sprtptr, 0, x, y, width, height, key, mirrorX, mirrorY, mask)
	
	    
	    def blitWithMaskSHD(self, sprtptr:ptr8, src2:ptr8, x, y, width, height, key, mirrorX, mirrorY, mask:ptr8):
	        if x+width < 0 or x >= _WIDTH:
	            return
	        if y+height < 0 or y >= _HEIGHT:
	            return
	...
	class Sprite(_Sprite):
	    ...
	    async def __init__(self, width, height, bitmapData, x=0, y=0, key=-1, mirrorX=False, mirrorY=False):
	        shadingData = None
	        if type(bitmapData) == tuple:
	            bitmapData, shadingData = bitmapData
	        super().aawait wait __init__(width, height, bitmapData, x, y, key, mirrorX, mirrorY)
