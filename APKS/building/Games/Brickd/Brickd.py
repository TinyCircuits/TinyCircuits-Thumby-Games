...
async def main():
	...
	while(1):
	    ...
	    # MOVE BALL PADDLE
	    if (await thumby.buttonL.pressed() == True and padSprite.x > 0 ):
	        padSprite.x -= 1
	    if (await thumby.buttonR.pressed() == True and padSprite.x < 62 ): #72-width
	        padSprite.x += 1
	    ...
	    if lose == True or gameScore == 30:
	        ...
	        if await thumby.buttonA.pressed(): # go back to game menu
	            thumby.reset() 
	            
	        elif await thumby.buttonB.pressed(): # Re-initialize values of variables to play again
	            lose = False  
	            gameScore = 0 
	            loopCtr = 0 
	            ballDir = 2
