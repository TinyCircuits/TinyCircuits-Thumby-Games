...
class Key:
    ...
    def display( self, x, y ):
        if self.character == ' ':
            ...
            d.drawText( '[sp]', x + 1, y + 2, 0 if self.selected else 1 )
            te_utility.defaultFont()
            return 16
        if self.selected:
            d.drawFilledRectangle( x, y, 7, 8, 1 )
        d.drawText( self.character, x + 1, y, 0 if self.selected else 1 )
        return 7
...
class Keyboard:
    def __init__( self, getSprite, text, position, layer = 0 ):
        self.layers = [
