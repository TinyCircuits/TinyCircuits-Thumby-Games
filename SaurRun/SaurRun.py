...
PlayerSpr = (0x04 ^ 0xFF, 0x08 ^ 0xFF, 0xC8 ^ 0xFF, 0xBC ^ 0xFF, 0x1C ^ 0xFF, 0x0E ^ 0xFF, 0x1A ^ 0xFF, 0x2C ^ 0xFF)
PlayerRunFrame1 = (0xFF, 0xFF, 0xFF, 0xFD, 0xF9, 0xBB, 0xBB, 0xD3, 0xE1, 0xF1, 0xC1, 0xB3, 0x61, 0xD5, 0xF3, 0xFF)
PlayerRunFrame2 = (0xFF, 0xFF, 0xF7, 0xFB, 0xFB, 0xFB, 0x3B, 0x93, 0xE3, 0x71, 0x03, 0xE7, 0xC3, 0xAB, 0xE7, 0xFF) 
CactusSpr1 = (0x00 ^ 0xFF, 0xFC ^ 0xFF, 0x86 ^ 0xFF, 0x92 ^ 0xFF, 0xC2 ^ 0xFF, 0xFC ^ 0xFF, 0x00 ^ 0xFF, 0x00 ^ 0xFF)
CactusSpr2 = (0x00 ^ 0xFF, 0x1E ^ 0xFF, 0x10 ^ 0xFF, 0xFE ^ 0xFF, 0xE4 ^ 0xFF, 0x20 ^ 0xFF, 0x78 ^ 0xFF, 0x00 ^ 0xFF)
CloudSpr = (0x9F, 0x4F, 0x63, 0x59, 0xBD, 0x73, 0x73, 0x65, 0x5C, 0x7E, 0x7E, 0x51, 0x57, 0x4F, 0x1F, 0xBF)

CactusSpr = CactusSpr1

thumby.display.fill(0)
thumby.display.drawText("Tinysaur", 0, 0)
thumby.display.drawText("  Run", 0, 8)
...
while(GameRunning):
    ...
    if(CactusPos < 8 and CactusPos > -8 and YPos > -8):
        ...
        thumby.audio.stop()
        thumby.display.drawText("Oh no!", 16, 8, 0)
        thumby.display.drawText(str(int(Distance))+"m", 16, 16, 0)
        thumby.display.drawText("Again?", 16, 24, 0)
    ...
    # Draw game state
    thumby.display.fill(1)
    thumby.display.blit(CactusSpr, int(16 + CactusPos), 24, 8, 8, 1)
    thumby.display.blit(CloudSpr, int(16 + CloudPos), 8, 16, 8, 1)

    if(t0 % 250000 < 125000 or YPos != 0.0):
        # Player is in first frame of run animation
        thumby.display.blit(PlayerRunFrame1, 8, int(23 + YPos), 16, 8, 1)
    else:
        # Player is in second frame of run animation
        thumby.display.blit(PlayerRunFrame2, 8, int(24 + YPos), 16, 8, 1)

    thumby.display.fillRect(0, 31, thumby.DISPLAY_W, 9, 0) # Ground
