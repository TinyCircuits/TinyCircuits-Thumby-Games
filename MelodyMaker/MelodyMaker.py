...
class Note:
    ...
    def draw(self):
        ...
        if self.octave:
            thumby.display.drawFilledRectangle(self.xPos, self.yPos
                    + len(self.notes) - self.idx, 9, 11, 1)
            thumby.display.drawText(self.notes[self.idx], self.xPos
                                    + 2, self.yPos + len(self.notes)
                                    - self.idx + 2, 0)
        else:
            thumby.display.drawRectangle(self.xPos, self.yPos
                    + len(self.notes) - self.idx, 9, 11, 1)
            thumby.display.drawText(self.notes[self.idx], self.xPos
                                    + 2, self.yPos + len(self.notes)
                                    - self.idx + 2, 1)
...
def drawCursor(curNote, color):
    if curNote < 8:
        thumby.display.drawLine(noteSpots[curNote].xPos, 19,
                                noteSpots[curNote].xPos + 7, 19, color)
    else:
        thumby.display.drawLine(noteSpots[curNote].xPos, 39,
                                noteSpots[curNote].xPos + 7, 39, color)
...
while 1:
    ...
    if thumby.buttonU.pressed() and not pressed:
        ...
        and not thumby.buttonL.pressed() \
        and not thumby.buttonR.pressed() \
        and not thumby.buttonA.pressed() \
        and not thumby.buttonB.pressed():
        pressed = False
